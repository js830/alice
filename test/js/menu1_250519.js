$(document).ready(function(){
    ...
    // 문서가 로딩되고 단 1번 실행
    let device_status //현재 디바이스가 pc모드인지 mobile모드인지 지정
    let win_w // 브라우저 넓이
    device_chk()// 함수의 실행
    ...
    */
    ...
    /*
        ...
        3. 이미 하나가 열려있는 상태에서 다른 메뉴를 클릭하면
           이전에 열려있던 메뉴는 닫힘
        ---> 메뉴를 클릭했을때
             메뉴가 열려있던 상태라면 나 자신을 닫고 끝남
             메뉴가 닫혀있는 상태라면 다른 메뉴를 닫고 나만 열음
             :: 메뉴를 클릭했을때 현재 메뉴가 여려있는지 닫혀있는지 알아야함
        ...
        4. 모바일에서 1차메뉴를 클릭하면 1차메뉴 a에 입력되어 있는 링크주소로 이동
           ...
           $(this).parents('li') 로 클릭한 a 의 상위요소 li를 선택
    */
